                ...
                <div v-else class="bg-boba-900/90 border border-boba-800/60 p-2.5 rounded-lg text-[10px] text-slate-300 space-y-2">
                  ...
                  <div v-else-if="tc.name === 'write_file'" class="space-y-1.5 font-mono">
                    <div class="text-emerald-300">
                      ✏️ Tulis File: <span class="text-white font-semibold">{{ tc.args.path }}</span>
                      <span class="text-slate-400 text-[9px] ml-1.5">({{ (tc.args.content || '').length }} bytes)</span>
                    </div>
                    <!-- Diff / Preview Box -->
                    <div class="bg-black/70 border border-boba-800 rounded p-2 text-[10px] max-h-36 overflow-y-auto font-mono text-emerald-200 whitespace-pre-wrap select-all">
                      <div class="text-[9px] text-slate-500 mb-1 border-b border-white/10 pb-0.5">Konten Baru yang Akan Disimpan (Backup otomatis: .boba.bak):</div>
                      {{ (tc.args.content || '').slice(0, 1500) }}{{ (tc.args.content || '').length > 1500 ? '\n... (konten panjang dipotong)' : '' }}
                    </div>
                  </div>
                  <div v-else-if="tc.name === 'run_security_audit'" class="text-amber-300 font-mono">
                    🛡️ Audit Keamanan: Cakupan <span class="text-white uppercase font-bold">{{ tc.args.scope || 'FULL' }}</span>
                  </div>
                  <div v-else-if="tc.name === 'setup_security_hardening'" class="text-sky-300 font-mono">
                    🔒 Hardening Server: SSH Port {{ tc.args.ssh_port || 22 }} (UFW & Fail2ban safeguard)
                  </div>
                  <div v-else class="text-slate-400 font-mono">{{ JSON.stringify(tc.args) }}</div>
                </div>
...
function handleExportRunbook() {
  const thread = aiStore.activeThread;
  if (!thread || !thread.messages || thread.messages.length === 0) {
    dialogStore.showToast('Belum ada pesan untuk diekspor', 'warning', 2000);
    return;
  }

  // Generate Markdown SOP & Shell commands
  const lines: string[] = [
    `# Runbook: ${thread.title || 'AI Server Session'}`,
    `Generated on: ${new Date().toISOString()}`,
    `Session ID: ${thread.sessionId}`,
    '',
    '## Step-by-Step Executed Log',
    '',
  ];

  const shCommands: string[] = [
    '#!/usr/bin/env bash',
    '# Generated by BOBA AI Copilot',
    `# Runbook: ${thread.title || 'AI Session'}`,
    'set -e',
    '',
  ];
  ...
  for (const m of thread.messages) {
    if (m.role === 'user') {
      ...
      if (m.toolCalls && m.toolCalls.length > 0) {
        for (const tc of m.toolCalls) {
          lines.push(`#### ⚡ Action: \`${tc.name}\` (Status: ${tc.status})`);
          if (tc.name === 'exec_command' && tc.args?.command) {
            lines.push('```bash', tc.args.command, '```');
            if (tc.status === 'completed') {
              shCommands.push(`# ${tc.args.description || tc.args.command}`);
              shCommands.push(tc.args.command);
            }
          } else {
            lines.push('```json', JSON.stringify(tc.args, null, 2), '```');
          }
          if (tc.result) {
            lines.push('**Output:**', '```text', String(tc.result).slice(0, 1000), '```\n');
          }
        }
      }
    }
  ...
  a.href = mdUrl;
  a.download = `runbook-${thread.id || 'boba'}.md`;
  a.click();
  URL.revokeObjectURL(mdUrl);

  dialogStore.showToast('Runbook Markdown (.md) berhasil diunduh', 'success', 2500);
}
